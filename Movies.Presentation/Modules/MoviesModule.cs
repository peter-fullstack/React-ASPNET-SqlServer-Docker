...
public static class MoviesModule
{
    public static void AddMoviesEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        (
            ...
            {
                var paginatedMovieDtos = await mediator.Send
                (
                    new GetMoviesQuery
                    (
                ...
                return Results.Extensions.OkPaginationResult
                (
                    paginatedMovieDtos.PageSize,
                    paginatedMovieDtos.CurrentPage,
                    paginatedMovieDtos.TotalCount,
                    paginatedMovieDtos.TotalPages,
                    paginatedMovieDtos.Items);
            }
        )
        .WithTags("Movies");
       ...
       .WithTags("Movies");
        ...
        app.MapPut("/api/movies/{id}", async 
        (
            IMediator mediator,
            int id,
        ...
        app.MapDelete("/api/movies/{id}", async 
        (
            IMediator mediator, 
            int id, 
            CancellationToken ct) =>
